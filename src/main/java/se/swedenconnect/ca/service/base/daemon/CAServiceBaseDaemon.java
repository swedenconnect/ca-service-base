 ...
 *
 * ca-service.config.remove-expired-certs=false; ca-service.config.remove-expired-grace-seconds=86400;
...
public class CAServiceBaseDaemon implements ApplicationEventPublisherAware, InitializingBean {
   ...
   */
  @Autowired
  public CAServiceBaseDaemon(final CAServices caServices,
      @Value("${ca-service.config.remove-expired-certs}") final boolean deleteExpired,
      @Value("${ca-service.config.remove-expired-grace-seconds}") final int gracePeriodSeconds,
      @Value("${ca-service.config.daemon-timer-seconds}") final int daemonTimerSeconds) {
   ...
   */
  @Scheduled(initialDelayString = "${ca-service.config.daemon-timer-seconds}"
      + "000", fixedDelayString = "${ca-service.config.daemon-timer-seconds}" + "000")
  public synchronized void deleteExpiredCertificates() throws IOException {
    if (this.deleteExpired) {
      ...
      final List<String> caServiceInstanceList = this.caServices.getCAServiceKeys();
      for (final String instance : caServiceInstanceList) {
        ...
        final List<BigInteger> deletedCertSerials =
            caService.getCaRepository().removeExpiredCerts(this.gracePeriodSeconds);
        ...
        if (deletedCertSerials != null) {
          ...
          for (final BigInteger certSerial : deletedCertSerials) {
            this.applicationEventPublisher
                .publishEvent(AuditEventFactory.getAuditEvent(AuditEventEnum.expiredCertDeleted,
