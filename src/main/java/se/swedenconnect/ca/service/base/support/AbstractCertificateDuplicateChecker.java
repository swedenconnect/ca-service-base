...
public abstract class AbstractCertificateDuplicateChecker implements CertificateDuplicateChecker {
  ...
  @SneakyThrows
  @Override public String getCertId(X509Certificate certificate) {
    if (certificate == null){
      return null;
    }
  ...
  @Override public List<X509Certificate> removeEquivalentCerts(List<X509Certificate> trustedCertList) {
    Map<String, X509Certificate> uniqueCertsMap = new HashMap<>();
    for (X509Certificate certificate : trustedCertList) {
    ...
    log.debug("Found {} duplicates", trustedCertList.size() - uniqueCertsMap.size());
